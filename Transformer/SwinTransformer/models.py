...
class BasicLayer(nn.Module):
    def __init__(self, dim, input_resolution, depth, heads, window_size, dropout_path, ff_ratio=4., qkv_bias=True,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution, heads=heads, window_size=window_size,
        ...
        self.downsample = downsample(dim=dim, input_resolution=input_resolution, norm=norm) if downsample else None
...
class SwinTransformer(nn.Module):
    def __init__(self, image_size=224, patch_size=4, in_channels=3, num_classes=10, embed_dim=96,
                 depths=[2, 2, 6, 2], heads=[3, 6, 12, 24], window_size=7, ff_ratio=4., qkv_bias=True,
                 dropout=0., attn_dropout=0., dropout_path=0.1, norm=nn.LayerNorm, patch_norm=True, ape=False):
        super().__init__()
